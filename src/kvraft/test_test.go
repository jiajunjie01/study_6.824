...
import "6.824-golabs-2020/src/porcupine"
import "6.824-golabs-2020/src/models"
import "testing"
import "strconv"
import "time"
import "math/rand"
...
// 基本测试如下:一段时期中一个或者多个clients提交Append/Get操作给servers.
// 在这段时间后,test检查所有追加的values是在场的,按特定key的顺序排列的.
// 如果unreliable被设置,RPC可能失败.如果crash被设置,这个servers也会一段时间后挂掉重启.如果partitions被设置
// test同时重分servers和client的网络分区.
// maxraftstate如果是正数,则需要snapshot.
